...
test("site shell, footer, and prototype generators are present", async () => {
  ...
  assert.match(footerJs, /data-escalate-print-job/);
  ...
  assert.match(printHtml, /class="upload-dropzone"/);
  assert.match(printHtml, /Browse or drop an STL file/);
  assert.match(printHtml, /Max 12MB with secure storage/);
  assert.match(printHtml, /id="saveUploadButton"/);
  assert.match(printHtml, /id="savedUploads"/);
  assert.match(printJs, /stlBase64/);
  assert.match(printJs, /maxUploadBytes = 12_000_000/);
  assert.match(printJs, /function parseStlMesh\(buffer\)/);
  assert.match(printJs, /function parseAsciiStlMesh\(text\)/);
  assert.match(printJs, /function renderUploadedEnvelopePreview\(svg, bounds, colour, view = \{\}\)/);
  ...
  assert.match(printJs, /function saveUploadedStl\(\)/);
  assert.match(printJs, /accountService\.uploadStlFile/);
  assert.match(printJs, /accountService\.downloadStlFile/);
  assert.match(printJs, /delete parameters\.stlBase64/);
  assert.match(printJs, /storagePath/);
  assert.doesNotMatch(printJs, /Prototype upload limit is 650KB/);
  assert.match(printJs, /accountService\.upsertDesign/);
  assert.match(printJs, /saved_upload: true/);
  assert.match(printJs, /function isSavedStlUpload\(upload\)/);
  ...
  assert.match(stitchHtml, /data-stitch-style="thread-slot-tray"/);
  assert.match(stitchHtml, /data-stitch-style="floss-card"/);
  assert.doesNotMatch(stitchHtml, /id="layoutStyle"/);
  ...
  assert.doesNotMatch(stitchJs, /embeddedTrayWidth/);
  assert.match(stitchJs, /engravedLabels/);
  assert.match(stitchJs, /function flossCardPreviewGeometry\(config\)/);
  assert.match(stitchJs, /function threadSlotTrayPreviewGeometry\(config\)/);
